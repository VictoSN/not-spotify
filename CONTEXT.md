...
- **Toasts:** `src/utils/toast.ts` exports `notify.{success,error,info}` (wraps `sonner`; outlet is `AppToaster` in `App.tsx`). Use it to surface caught errors instead of silent `catch {}`. Dead settings toggles (`language`/`quality`/`normalize`/`crossfade`) are intentionally **disabled with a "Coming soon" badge** in `SettingsPage` until wired — don't re-enable without implementing them.
- **Confirmations:** use the promise-based `useConfirm()` hook from `src/hooks/useConfirm.ts`; its single `ConfirmProvider` is mounted in `App.tsx`. Do not add native `confirm()` calls. The Headless UI dialog provides focus trapping, safe Cancel autofocus, Esc/backdrop cancellation, and focus restoration.
- **Rate limiting:** `Program.cs` defines fixed-window `auth` (20/minute per remote IP) and `chat-send` (20/10 seconds per authenticated user) policies. `AuthController` applies `auth` at controller level; only `ChatController.Send` applies `chat-send`. Rejections are JSON 429 responses with `Retry-After`; `chatStore` surfaces the message via toast.
- **Downloads:** `AudioDownloadService` is the single server-side audio reader for track downloads and album/playlist ZIPs (storage key first; absolute legacy URL fallback). `GET /tracks/{id}/download` allows Premium listeners plus admins/the owning artist; all frontend track-download buttons call `trackService.download()` rather than linking to `audioUrl`. `Content-Disposition` is exposed through CORS so the browser preserves the real extension/filename.
