...
5. ✅ **Notifications center** — DONE (2026-06-14, commit aeec82f). New `Notification` entity + `AddNotifications` migration (applied to shared DB); bell + dropdown in TopBar, live via SignalR + poll fallback. Producers: friend-request received/accepted (admin approve/reject producers are a trivial follow-up via `NotificationService`).
