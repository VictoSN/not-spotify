...
Last updated: 2026-06-14 (Tier 1 hardening — shared accessible ConfirmDialog)
...
- **Hardening/polish:** dead settings toggles (`language`, `quality`, `normalize`, `crossfade`) now **disabled with a "Coming soon" badge** instead of lying; global **toast** system (sonner, theme-aware) surfacing previously-silent failures (share, add/remove-playlist, offline save/remove); shared accessible **`ConfirmDialog`** replaces all native `confirm()` calls (focus trap, safe initial focus, Esc/cancel, focus restoration).
...
**Notes for next session:** Continue Tier 1 before Tier 2/3. Highest-value remaining work: backend fixed-window rate limiting on auth + chat-send, then unify single-track download with server download paths. Frontend a11y audit and Queue/Up-next remain; ConfirmDialog already covers confirmation-modal focus/Esc. Use `notify` for remaining user-facing silent catches.
