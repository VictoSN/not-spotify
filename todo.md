...
- [x] **Concert/tour info** — **done (2026-06-21)**, seeded (no paid concert API). `TourDate` model + idempotent Program.cs guard (`CREATE TABLE IF NOT EXISTS` + one-time seed of 3 future dates per artist across 8 aligned city/venue/country sets, with a generic web-search ticket link — a real, working link, not a fake checkout). `GET /artists/{id}/tour` returns upcoming dates. Frontend: `artistService.getTourDates` + an "On tour" panel on the artist page (date chip · city/region via `Intl.DisplayNames` · venue · Tickets link), en/es/fr. Backend + frontend build clean.
